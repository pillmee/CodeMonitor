...
- **주요 지표**:
    ...
    - **Net Change**: 기준 시점부터 '오늘'까지 발생한 코드량의 순증감 (절댓값).
    - **Refinement Ratio (vs Today)**: `(오늘의 LOC / 기준 시점의 LOC) * 100`. 기준 시점 대비 현재(오늘)의 코드 비중을 퍼센트로 표현. (예: 100% 보다 크면 코드 증가, 100% 미만이면 코드 감축/최적화 성공)
- **UI/UX 구현**:

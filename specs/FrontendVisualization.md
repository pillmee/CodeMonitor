...
- **Panning (X/Y 축 이동)**: 마우스 드래그를 통해 시간 축(X축)과 데이터 값 축(Y축)을 동시에 자유롭게 이동할 수 있음. 그래프의 선이 마우스 커서의 움직임을 그대로 따르는 직관적인 조작 방식(Natural Drag)을 제공함. 렌더링 성능 최적화를 통해 60fps 수준의 끊김 없는 조작감을 보장함.

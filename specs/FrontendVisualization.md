...
- **서버 동기화**: 지정된 비교 날짜는 백엔드 DB의 `settings` 테이블에 저장되어, 페이지를 새로고침하거나 다른 기기/브라우저에서 접속해도 동일한 비교 상태를 유지함.
- **차트 하이라이트**: 차트 영역 내에 두 시점을 잇는 **세로 점선**과 그 사이의 **반투명 하이라이트 영역**을 표시하여 비교 대상을 시각적으로 명확히 함.
